...
Supabase Cron can invoke SQL/database functions or HTTP/Edge Functions and records run status. Use it to dispatch/check small durable jobs, not a single unbounded global scrape. Edge Functions have execution limits, so persist progress, bound fetch/parse workloads and retry safely. An externally hosted health check should detect a failed or paused scheduler. On the free plan, frequent cron jobs must check for pending work inside the database before calling a function, because every request counts toward the 1 GB/month log ingestion allowance (implemented 2026-09-25 as `desk_scheduler_due()`). These are candidate designs awaiting the phase-zero test, not a deployed system. [Supabase Cron](https://supabase.com/docs/guides/cron), [Edge Function limits](https://supabase.com/docs/guides/functions/limits)
