...
Existing v1 assessments are not silently treated as v2 judgments. New discovery uses v2 immediately. The scheduler rescreens old records once per night, up to 300 a night (about 6,000 were queued on September 22, 2026), preserving feedback, saved status and review history. Rescreens pause for 30 minutes when Google rate-limits them. Manual searches retain pause/resume/cancel behavior. Legacy policy helpers remain for compatibility with stored v1-era structures; v2 assessments use the new signal policy.
